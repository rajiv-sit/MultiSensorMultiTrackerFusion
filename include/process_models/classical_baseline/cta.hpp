...
// ----------------------
// Coordinated Turn with Acceleration (CTA)
// ----------------------
class CTAModel : public IProcessModel {
public:
    explicit CTAModel(const CTAParams& p = {}) : params(p) {}
    ...
    Vec f(const Vec& x, double dt) const override {
        ...
        double yaw = x(3);
        double omega = x(4);
        double a_t = x(5); // tangential acceleration

        // Update velocity with tangential acceleration
    ...
    Mat F(const Vec& x, double dt) const override {
        ...
        J(2,5) = dt; // velocity affected by tangential acceleration
        J(3,4) = dt;
    ...
    std::unique_ptr<IProcessModel> clone() const override {
        return std::make_unique<CTAModel>(*this);
    }
