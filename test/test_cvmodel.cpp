...
// CVModelTest
//
// Unit tests for the Constant Velocity (CV) model in 2D and 3D.
// Tests cover state propagation, process noise, Jacobians, and cloning.
//------------------------------------------------------------------------------
class CVModelTest : public ::testing::Test {
protected:
    void SetUp() override {
        params.q_v = 0.01;       // default process noise
        cv2d.setAxis(2);         // configure 2D CV model
        cv3d.setAxis(3);         // configure 3D CV model
    }
...
TEST_F(CVModelTest, StatePropagation2D) {
    Eigen::VectorXd x(4); // [px, vx, py, vy]
    x << 0, 1, 0, 1;
    double dt = 1.0;
    ...
    EXPECT_NEAR(x_next(1), 1.0, 1e-9); // vx constant
    EXPECT_NEAR(x_next(2), 1.0, 1e-9); // py + vy*dt
    EXPECT_NEAR(x_next(3), 1.0, 1e-9); // vy constant
}

TEST_F(CVModelTest, StatePropagation3D) {
    Eigen::VectorXd x(6); // [px, vx, py, vy, pz, vz]
    x << 0, 1, 0, 2, 0, 3;
    double dt = 2.0;
...
TEST_F(CVModelTest, NegativeDt) {
    ...
    EXPECT_NEAR(x_next(1), 3.0, 1e-9);  // vx constant
    EXPECT_NEAR(x_next(2), -2.0, 1e-9); // py + vy*dt
    EXPECT_NEAR(x_next(3), 4.0, 1e-9);  // vy constant
}
...
TEST_F(CVModelTest, ProcessNoise2D) {
    ...
    EXPECT_TRUE(Q.isApprox(Q.transpose(), 1e-12)); // symmetry check
}
